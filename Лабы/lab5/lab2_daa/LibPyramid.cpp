...
//{
//	int fi = 50;					// угол сечения в градусах
//	double fi_rad = fi * pi / 180;	// угол сечения в радианах
//	double h = 0.75;				// высота сечения
//	double x0 = 0;
//	double y0 = 0;
...
void CPyramid::Draw(CDC& dc, CMatrix& PView, CRect& RW)
{
	...
	// если делим на 2, то сечение будет посередине ребер
	double x1 = (MasVert[0].x + MasVert[3].x) / 2;
	double x2 = (MasVert[1].x + MasVert[4].x) / 2;
	double x3 = (MasVert[2].x + MasVert[5].x) / 2;

	// формула: (x - x1) / (x2 - x1) = (y - y1) / (y2 - y1)
	// аналогичная формула: y = ((x - x1) * (y2 - y1)) / (x2 - x1)  +  y1
	Sechenie[0].x = x1;
